    ...
    const updatePresence = async () => {
      ...
      try {
        const participantRef = ref(
          db,
          `rooms/${roomId}/participantsList/${user.uid}`
        );
        await update(participantRef, {
          id: user.uid,
          displayName:
            user.displayName || `Guest ${Math.floor(Math.random() * 1000)}`,
      ...
      if (snapshot.exists()) {
        ...
        setRoom((prevRoom) => {
          if (prevRoom) {
            ...
            if (
              JSON.stringify(updatedRoom.currentlyPlaying) !==
                JSON.stringify(prevRoom.currentlyPlaying) ||
              Math.abs(
                updatedRoom.playbackState?.currentTime -
                  (prevRoom.playbackState?.currentTime || 0)
              ) > 5 ||
              updatedRoom.playbackState?.isPlaying !==
                prevRoom.playbackState?.isPlaying
            ) {
            ...
            setCanListen(
              updatedRoom.createdBy === user?.uid ||
                updatedRoom.allowOthersToListen
            );
            return updatedRoom;
          }
          ...
          setCanListen(
            roomData.createdBy === user?.uid || roomData.allowOthersToListen
          );
    ...
    try {
      const endpoint =
        searchEngine === 1
          ...
          : searchEngine === 2
          ? `/api/youtube/search2?q=${encodeURIComponent(searchQuery)}`
          : `/api/youtube/search3?q=${encodeURIComponent(searchQuery)}`;
      ...
      if (searchEngine === "primary") {
        toast.error(
          "Primary search failed. Trying alternative search engine..."
        );
        setSearchEngine("alternative");

        try {
          const altResponse = await fetch(
            `/api/youtube/alt-search?q=${encodeURIComponent(searchQuery)}`
          );
          if (altResponse.ok) {
        ...
        toast.error(
          "Search failed. Try switching to the primary search engine."
        );
      }
    } finally {
  ...
  const handleSaveSong = async () => {
    ...
    try {
      if (isSongSaved) {
        ...
        if (result.success) {
          const song = result.playlist.find(
            (s) => s.videoId === room.currentlyPlaying.videoId
          );
          if (song) {
      ...
      <div className="flex items-center justify-center min-h-screen text-white bg-black">
        <div className="w-full max-w-md p-6 bg-gray-900 border border-gray-800 rounded-lg">
          ...
          <p className="mb-4 text-gray-400">
            This name will be shown to others in the room.
          </p>
      ...
      <header className="flex items-center px-4 border-b h-14 border-white/10">
        ...
        <div className="flex items-center gap-2 ml-auto">
          <span className="text-sm text-gray-400">
            {room.participants}{" "}
            {room.participants === 1 ? "listener" : "listeners"}
          </span>
          {user?.isAnonymous && (
          ...
          {isCreator && (
            <div className="flex items-center gap-2 ml-4">
              ...
              <Switch
                checked={allowOthersToListen}
                onCheckedChange={toggleAllowOthersToListen}
              />
            </div>
          )}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="text-lg font-medium truncate">
                      {room.currentlyPlaying.title}
                    </h2>
                    <p className="text-sm text-gray-400">
                      {room.currentlyPlaying.channelTitle} • Added by{" "}
                      {room.currentlyPlaying.addedByName}
                  ...
                  {messages.map((message) => (
                    <div key={message.id} className="flex items-start gap-2">
                      <Avatar className="w-8 h-8">
                        <AvatarFallback
                          className={message.isAnonymous ? "bg-yellow-800" : ""}
                        >
        ...
        <div className="hidden">
          {canListen && room?.currentlyPlaying && (
            <div
              key={room.currentlyPlaying.videoId}
