...
export function SearchResults({ video, onAdd }: SearchResultsProps) {
  return (
    <div className="flex items-center gap-3 p-2 rounded-md hover:bg-white/5 hover:cursor-pointer w-[375px]" onClick={onAdd}>
      <img
      ...
      <button onClick={onAdd} className="px-2 py-1 text-xs rounded bg-white/10 hover:bg-white/20">
        Add
      </button>
    </div>
  )
